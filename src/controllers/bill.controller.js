...
const Revenue = async (req, res, next) => {
    try {
        ...
        const option = type == "bill" ? {createdAt: {
            $gt: dateStart,
            $lt: dateEnd
        },
        'status.0.statusTimeline' : { $in :'Done'}} : { createdAt: {
            $gt: dateStart,
            $lt: dateEnd
        } }
        //@ts-ignore
        tempModel.find(option).exec((err, bills) => {
            ...
            if(bills.length == 0) return responseSuccess({res, message: config.success, data: { graph: [], products: [] }}) 
            // Gom nhom du lieu
            var counter = {}
            var graph = []
...
const shipCalculate = async (address, weight, value) => {
    ...
    try {
        const ghtk = await axios.get(process.env.GHTK_URL + fromObject(data), {headers: { "Token": `${process.env.GHTK_API_TOKEN}` }})
...
const SubBill = async (req, res, next) => {
    try {
        ...
        var result_discount = await discountCalculate(discountCode, cartItems, total, ship, account ? account._id.toString() : "")
        reduce += result_discount.value
        warning += result_discount.error

        res.send({ msg: config.message.success, warning, data: { cart_details: cartItems, ship, total, discount: reduce, discountCode, address } })
    } catch (err) {
        console.log(err)
...
const Create = async (req, res, next) => {
    try {
        ...
        if (cod == undefined)
            return res.status(400).send({status: 400 ,msg: "Mời chọn phương thức thanh toán" })
        if (!phone)
            return res.status(400).send({status: 400 ,msg: "Thiếu số điện thoại. " })
        if (!!account) {
        ...
        const products = []
        cartItems.forEach(i => products.push({ product: i.product, color: i.color, quantity: i.quantity, price: i.price, sale: i.sale }))
        const status = [{ statusTimeline : "Ordered", time: Date.now()}]
        const bill = new Bill({ account: account._id, phone: phone, address, products, discountCode, ship, total, discount: reduce, verify,status })
        ...
        try {
            ...
            await Account.findByIdAndUpdate(bill.account._id.toString(), { $push: { "notifications": { $each: [{message :ms_sc}], $position: 0 } } }).select("_id").exec()
            if (cod == true)
            ...
            await Account.findByIdAndUpdate(account._id.toString(), { $push: { "notifications": { $each: [{message :ms_fl}], $position: 0 } } }).select("_id").exec()
    ...
    try {
        ...
        if (list.includes(status))
        {
            ...
            if (bills.includes(_id)) {
                ...
                return res.status(400).send({ msg: config.message.errPermission })
        ...
        try {
            ...
            if (status == "Canceled")
                ...
                console.log("bill",bill)
                if (!(await Account.findByIdAndUpdate(bill.account._id, { $push: { "rate_waits": { $each: products } } }, opts).exec()))
                    throw Error("Fail")
            }
            ...
            await Account.findByIdAndUpdate(bill.account._id.toString(), { $push: { "notifications": { $each: [{message}], $position: 0 } } }).select("_id").exec()
            return responseSuccess({ res, message: config.message.success })
        } catch (error) {
            console.log(error)
            await session.abortTransaction();
